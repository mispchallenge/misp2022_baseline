...
def median_filtering(session_label, padding="reflect", taps=51):
    filtered = {}
    for session in session_label.keys():
        signal = session_label[session]
        ...
        filtered[session] = np.array(filterd_signal)
    return filtered

def mean_filtering(session_label, padding="reflect", taps=51):
    filtered = {}
    for session in session_label.keys():
        signal = session_label[session]
        if signal.shape[1] <= taps//2:
        ...
        for i in range(taps//2, taps//2+signal.shape[1]):
            # print(np.median(pad_signal[i -taps//2:i +taps//2],axis =1))
            filterd_signal[:, i-taps//2] = np.mean(pad_signal[:, i-taps//2: i+taps//2+1], axis=1)
        filtered[session] = np.array(filterd_signal)
    return filtered

def rttm_filter_with_vad(rttm, oracle_vad, oracle_vad_list):
    for session in rttm.keys():
        silence = 0
        num_speaker = rttm[session].shape[0]
        for spk in range(num_speaker):
            silence += rttm[session][spk]
        for spk in range(num_speaker):
            rttm[session][spk] = rttm[session][spk] * oracle_vad[session][:len(rttm[session][spk])]
        for l in oracle_vad_list[session]:
            if np.sum(silence[l[0]:l[1]]==0) > 0:
                max_spk = spk
                max_len = np.sum(rttm[session][spk][l[0]:l[1]])
                for spk in range(num_speaker):
                    if np.sum(rttm[session][spk][l[0]:l[1]]) > max_len:
                        max_spk = spk
                        max_len = np.sum(rttm[session][spk][l[0]:l[1]])
                rttm[session][max_spk][l[0]:l[1]][silence[l[0]:l[1]]==0] = 1
    return rttm

def write_rttm(session_label, output_path, fps):
    #print(session_label.keys())
    with open(output_path, "w") as OUT:
        for session in session_label.keys():
            #print(np.sum(session_label[session]))
            num_speaker = session_label[session].shape[0]
            for k in range(num_speaker):
                i = 0
                while i < len(session_label[session][k]):
                    if session_label[session][k][i] == 1:
                        start = i
                        durance = 1
                        i += 1
                        while i < len(session_label[session][k]) and session_label[session][k][i] == 1:
                            i += 1
                            durance += 1
                        ...
                        OUT.write("SPEAKER {} 1 {:.2f} {:.2f} <NA> <NA> {} <NA> <NA>\n".format(session, 1.0*start/fps, 1.0*durance/fps, str(k)))
                    i += 1
...
def main(args):
    ...
    for l in prob_array:
        ...
        # R03_S102103104_C08_I0_Middle_104-25364-37719
        session = l.split("-")[0]
        end = int(l.split("-")[-1].split(".")[0])
    ...
    for l in prob_array:
        ...
        if session not in session_label.keys():
            session_label[session] = np.zeros([prob_label.shape[0], MAX_DUR[session]])
        prob_label = np.load(os.path.join(args.prob_array_dir, l))
        #print(prob_label[:, 1].shape)
        #print(session_label[session][speaker][start:end].shape)
        session_label[session][:, start:end][prob_label[:, :(end-start), 1] > args.threshold / 100] = 1
        #print(np.sum(session_label[session.split('.')[0]]))
        #session_label[session.split('.')[0]] = np.argmax(np.load(os.path.join(prob_array_dir, session)), axis=2)
    #output_path = os.path.join(args.prob_array_dir, "rttm_th{:.2f}".format(args.threshold/100))
    if args.meanfilter != -1:
        session_label = mean_filtering(session_label, taps=args.meanfilter)
        #output_path = output_path + "_meanfilter{}".format(args.meanfilter)
    if args.medianfilter != -1:
        session_label = median_filtering(session_label, taps=args.medianfilter)
        #output_path = output_path + "_medianfilter{}".format(args.medianfilter)

    oracle_vad = None
    if os.path.isfile(args.oracle_vad):
        oracle_vad, oracle_vad_list = get_oracle_vad_mask(args.oracle_vad)
        #session_label = rttm_filter_with_vad(session_label, oracle_vad, oracle_vad_list)
        ...
        for session in session_label.keys():
                ...
                for k in range(num_speaker):
                    pass
                    try:
                        session_label[session][k] = session_label[session][k] * oracle_vad[session][:len(session_label[session][k])]
                    except:
                        print(session)

    write_rttm(session_label, os.path.join(args.prob_array_dir, "rttm_th{:.2f}".format(args.threshold/100)), args.fps)
...
def make_argparse():
    ...
    parser.add_argument('--prob_array_dir', metavar='DIR', required=True,
                        help='prob_array_dir.')
    parser.add_argument('--fps', metavar='Int', type=int, default=100,
                        help='meanfilter.')
