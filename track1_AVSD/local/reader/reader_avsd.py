...
class Audio_AEmbedding_VEmbedding_Worse_Data_Reader():
    ...
    def get_silent_video_segments(self, video_scp):
        ...
        with open(video_scp) as SCP_IO:
            for l in SCP_IO:
                '''
                R03_S090091_C12_I2_Middle_091-8550-8750.htk
                '''
                session = "_".join(os.path.basename(l).split("_")[:-2])
                speaker, start, end = os.path.basename(l).split("_")[-1].split(".")[0].split("-")
                start = int(start)
    ...
    def get_feature_info(self, audio_dir, video_scp, audio_type):
        ...
        for session in tqdm.tqdm(embeddings.keys()):
            try:
                ...
                for s in audio_type:
                    for ch in range(6):
                        total_frame = HTK.readHtk_info(os.path.join(audio_dir, f"{session}_{s}_ch{ch}.fea"))[0] // 4 
            ...
            for utt in embeddings[session].keys():
                ...
                for s in audio_type:
                    for ch in range(6):
                        same_session_feature_list[session].append(self.worse_data([session, utt, embeddings[session][utt], os.path.join(audio_dir, f"{session}_{s}_ch{ch}.fea")]))
            feature_list.extend(same_session_feature_list[session])
        total_item = self.normal + self.worse_speech + self.worse_silence
        print(f"Normal:{self.normal / total_item} Set_silence:{self.worse_silence / total_item} Set_speak:{self.worse_speech / total_item}")
