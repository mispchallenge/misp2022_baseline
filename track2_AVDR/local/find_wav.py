...
def find_wav(data_root, scp_dir, scp_name='wpe', wav_type='Far', n_split=1):
    ...
    # wav_dir = os.path.join(data_root, "audio", "train", wav_type.lower())
    wav_dir = os.path.join(data_root, wav_type.lower())
    # all_wav_paths = [os.path.join(wav_dir,i) for i in os.listdir(wav_dir)] 
    # all_wav_paths = glob.glob(os.path.join(wav_dir, '*.wav'))
    all_wav_paths = glob.glob(os.path.join(data_root, '*.wav'))
    ...
    if n_split == 1:
        with codecs.open(os.path.join(scp_dir, '{}.1.scp'.format(scp_name)), 'w') as handle:
